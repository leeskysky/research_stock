...
# 딕셔너리를 이용하여 데이터 생성
    ...
    if stock.shape[0] == 981: # nan값 있는 기업 방지
        ...
        new_change = stock['Change'].shift(-1)

        ndays_high = stock['High'].rolling(window=14, min_periods=1).max() # 14일 중 최고가 
        ndays_low = stock['Low'].rolling(window=14, min_periods=1).min() # 14일 중 최저가 
        fast_k = (stock['Close'] - ndays_low) / (ndays_high - ndays_low)  # Fast %K 구하기 
        slow_d = fast_k.rolling(window=3).mean()
        
        rsi = ta.momentum.rsi(stock['Close'])
        macd = ta.trend.macd(stock['Close'])
        macdsignal = ta.trend.macd_signal(stock['Close'])
        obv = ta.volume.on_balance_volume(stock['Close'],stock['Volume'])
                 
        for row, ma_5, ma_20, ma_60, ma_120, new_change, fast_k, slow_d,rsi,macd,macdsignal,obv in zip(lst_stock, ma_5, ma_20, ma_60, ma_120, new_change, fast_k, slow_d,rsi,macd,macdsignal,obv):
            ...
            if Date < pd.to_datetime('2018-01-02') or Date == pd.to_datetime('2020-12-30'): # 18년 데이터부터 생성
                continue
            ...
            if Volume == 0 or abs(Change) > 0.3 : # 거래정지일 & 이상치는 제거함.
                ...
                new_stock_lst.append([Date, code, Open, High, Low, Close, Change, Volume, Scaled_vol, trading_value, ma_5, ma_20, ma_60, ma_120, new_change,fast_k,slow_d,rsi,macd,macdsignal,obv])
